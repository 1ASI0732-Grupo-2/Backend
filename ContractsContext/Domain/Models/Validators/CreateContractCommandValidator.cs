...
public class CreateContractCommandValidator: AbstractValidator<CreateContractCommand>
{
     public CreateContractCommandValidator()
    {
        RuleFor(x => x.OfficeId)
            .NotEmpty()
            .WithMessage("El ID de la oficina es requerido.");
        ...
        RuleFor(x => x.RenterId)
            .NotEmpty()
            .WithMessage("El ID del arrendatario es requerido.");

        RuleFor(x => x.RenterId)
            .NotEqual(x => x.OwnerId)
            .WithMessage("El propietario y el arrendatario no pueden ser la misma persona.");
        ...
        RuleFor(x => x.BaseAmount)
            .GreaterThan(0)
            .WithMessage("El monto base debe ser mayor a cero.");

        RuleFor(x => x.LateFee)
            .GreaterThanOrEqualTo(0)
            .WithMessage("La penalidad por mora no puede ser negativa.");

        RuleFor(x => x.InterestRate)
            .GreaterThanOrEqualTo(0)
            .WithMessage("La tasa de interés no puede ser negativa.")
            .LessThanOrEqualTo(100)
            .WithMessage("La tasa de interés no puede exceder el 100%.");
    }
